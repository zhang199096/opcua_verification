...
namespace CSharpTcpDemo.com.dobot.api
{
    public class DobotMove : DobotClient
    {
        ...
        public string GetPathStartPose(string script)
        {
            if (!IsConnected())
            {
                return "device does not connected!!!";
            ...
            string str = String.Format("GetPathStartPose({0})", script);
            if (!SendData(str))
            {
                return str + ":send error";
            }
        ...
        public string StartPath(string script,int _const,int cart)
        {
            if (!IsConnected())
            {
                return "device does not connected!!!";
            ...
            string str = String.Format("StartPath({0},{1},{2})", script,_const,cart);
            if (!SendData(str))
            {
                return str + ":send error";
            }
